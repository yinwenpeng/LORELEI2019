from load_data import load_BBN_multi_labels_dataset, load_single_test_sample, load_official_testData_il_and_MT, generate_2019_official_output, load_trainingData_types_plus_others,load_trainingData_types,load_SF_type_descriptions, average_f1_two_array_by_col, load_fasttext_multiple_word2vec_given_file, load_word2vec_to_init
...
bert_tokenizer = BertTokenizer.from_pretrained('/shared/experiments/kkarthi/pretrained_models')#load the vocab.txt file
bert_model = BertModel.from_pretrained('/shared/experiments/kkarthi/pretrained_models')
bert_model.eval()
bert_model.to('cuda')

config = {"emb_size": 300, "hidden_size": 300, 'epoch_num': 4, 'lr': 0.0003, 'batch_size': 50,
...
class Encoder(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, pretrained_embeddings, batch_size):
        ...
        if use_bert:
            self.hidden2tag = nn.Linear(768*4, tagset_size)
            self.hidden2tag_task2 = nn.Linear(768*4, 16)
        else:
    ...
    def forward(self, sentence, seq_lengths, mask, label_sent, label_mask, bert_rep_batch):
        ...
        if use_bert:
            ...
            combine_rep_batch = torch.cat([bow, aconv_output,aconv_output2, conv_output, bert_rep_batch], 1).tanh()
...
def get_mask_demo(data, masks, minibatch):
    '''repeat the test instance into batch size'''
    data_batch = [data[0] for x in range(minibatch)]
    mask_batch = [masks[0] for x in range(minibatch)]
    # label_batch = [labels[x] for x in minibatch]
    sentence_batch = np.array(data_batch)
    mask_batch = np.array(mask_batch)
    # label_batch = np.array(label_batch)
    ...
    for epoch in range(epoch_num):
        ...
        for i, minibatch in enumerate(minibatches_idx):
            ...
            if iter %20 == 0:
                ...
                print('testing....')
                test(testing_data_index, testing_masks, model, label_sent, label_mask, id2word)
    print('train over.')
...
def run_demo(test_sent):


    word2id={}
    test_file_path = '/home/wyin3/LORELEI/2019/retest/il3_uyghur_labeled_as_training_seg_level.txt'
    ...
    test_sents, test_masks, word2id = load_single_test_sample(word2id, config['maxSentLen'], test_sent)
    ...
    label_sent = autograd.Variable(torch.cuda.LongTensor(label_sent))
    label_mask = autograd.Variable(torch.cuda.FloatTensor(label_mask))
    result = test(testing_data_index, testing_masks, model, label_sent, label_mask, id2word)

    return result

if __name__ == '__main__':
    print(run_demo('haha this is a test sentence'))
