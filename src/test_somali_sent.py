from load_data import load_BBN_multi_labels_dataset, load_il_groundtruth_as_testset, load_somali_input, load_trainingData_types_plus_others,load_trainingData_types,load_SF_type_descriptions, average_f1_two_array_by_col, load_fasttext_multiple_word2vec_given_file, load_word2vec_to_init
...
from preprocess_common import somali_get_type, validate_output_schema
...
# def train(task1_data,task2_data,test_data, label_sent, label_mask, id2word, epoch_num, model):
...
def test(training_data_index, training_masks, test_lines, model, label_sent, label_mask, id2word):
    '''replace save_model_path into your model path'''
    ...
    with torch.no_grad():
        ...
        for i, minibatch in enumerate(minibatches_idx):
            ...
            sentence_batch = autograd.Variable(torch.LongTensor(sentence_batch))
            mask_batch = autograd.Variable(torch.FloatTensor(mask_batch))
            ...
            # print('lengths_batch:', lengths_batch)
            ...
            recover_seq_idx=autograd.Variable(torch.LongTensor(np.array(recover_seq_idx)))
    ...
    # output_file_path = output_file_head+output_file_path_codes[0]+'.json'
    # print('generating ...', output_file_path)
    sent_type =  somali_get_type(test_lines, pred_types, pred_confs, pred_others)
...
if __name__ == '__main__':
    ...
    # bert_paths = ['/shared/experiments/kkarthi/Final_Bert_Models/il11/il11_1M',
    # '/shared/experiments/kkarthi/Final_Bert_Models/il11/il11_V1_1M']

    # emb_root = '/home1/w/wenpeng/dataset/LORELEI/'
    # emb_paths = [[emb_root+'cleaned-100k-SOM3-multicca.d300.eng.txt',
    #             emb_root+'cleaned-100k-SOM3-multicca.d300.SOM3.txt']]



    '''test setup'''
    # test_file_paths = '/home1/w/wenpeng/dataset/LORELEI/2019_09_07_somali-setE-as-test-input_edl0_filtered_w2.txt'
    '''
    ...
    # output_file_head = '/home1/w/wenpeng/dataset/LORELEI/somali_system_output_20190907_'
    '''setup over'''
    # if use_bert:
    #     print('loading bert...')
    ...
    '''put your somali sent here'''
    somali_sent_str = 'somali sentence'
    test_sents, test_masks, test_lines, word2id = load_somali_input(word2id, config['maxSentLen'], somali_sent_str)
    ...
    print("build model...")
    model= build_model(config['emb_size'], config['hidden_size'], vocab_size,
                                                  12, None, config['lr'], config['batch_size'])
