...
if use_bert:
    ...
    bert_model.eval()
    bert_model.to('cuda')
    bert2_tokenizer = BertTokenizer.from_pretrained('/shared/experiments/kkarthi/Extend_mbert/il9/il9_1M')#load the vocab.txt file
    bert2_model = BertModel.from_pretrained('/shared/experiments/kkarthi/Extend_mbert/il9/il9_1M')
    bert2_model.eval()
    bert2_model.to('cuda')
...
class Encoder(nn.Module):
    ...
    def __init__(self, embedding_dim, hidden_dim, vocab_size, tagset_size, pretrained_embeddings, batch_size):
        ...
        if use_bert:
            ...
            self.bert2_hidden2tag = nn.Linear(768*4, tagset_size)
            self.bert2_hidden2tag_task2 = nn.Linear(768*4, 16)
        else:
            self.hidden2tag = nn.Linear(self.emb_size+4*self.hidden_dim, tagset_size)
            self.hidden2tag_task2 = nn.Linear(self.emb_size+4*self.hidden_dim, 16)
    ...
    def forward(self, sentence, seq_lengths, mask, label_sent, label_mask, bert_rep_batch, bert2_rep_batch):
        '''
        sentence: (batch, len)
        '''
        '''label descriptions'''
        ...
        if not use_bert:
            ...
            bert2_features = bert2_rep_batch
            system2018_features = torch.cat([torch.cat([bow, aconv_output,aconv_output2, conv_output], 1).tanh()],1)
            ratio=[0.4,0.3,0.3]
            tag_prob = self.hidden2tag(system2018_features).sigmoid()*ratio[0]+self.bert_hidden2tag(bert_features).sigmoid()*ratio[1]+self.bert2_hidden2tag(bert2_features).sigmoid()*ratio[2]
            tag_prob_task2 = self.hidden2tag_task2(system2018_features).sigmoid()*ratio[0]+self.bert_hidden2tag_task2(bert_features).sigmoid()*ratio[1]+self.bert2_hidden2tag_task2(bert2_features).sigmoid()*ratio[2]
        ...
        for i, minibatch in enumerate(minibatches_idx):
            ...
            if use_bert:
                for sent in sent_list:
                    ...
                    bert2_rep = sent_to_embedding_last4(sent, bert2_tokenizer, bert2_model, True)
            ...
            tag_scores,_= model(sentence_batch, seq_lengths, mask_batch, label_sent, label_mask, bert_rep_batch, bert2_rep_batch)
            ...
            if i < len(minibatches_idx_task2):
                ...
                if use_bert:
                    ...
                    bert_rep_batch = torch.cat(bert_rep_batch, 0) #(batch, 768)
                    bert2_rep_batch = torch.cat(bert2_rep_batch, 0) #(batch, 768)
                tag_scores, tag_scores_task2= model(sentence_batch, seq_lengths, mask_batch, label_sent, label_mask, bert_rep_batch, bert2_rep_batch)
...
if __name__ == '__main__':
    ...
    config = {"emb_size": 300, "hidden_size": 300, 'epoch_num': 100, 'lr': 0.0003, 'batch_size': 50,
