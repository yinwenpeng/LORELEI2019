...
def Somali_into_test_filteredby_NER_2019(ltf_path, writefile_path, docid2entity_pos_list, window):
    '''
    docid2entity_pos_list: doc_id: ['12-14-kbid', '34-67-kbid', ...]
    '''
    # print('docid2entity_pos_list:', docid2entity_pos_list)
    # exit(0)
    writefile_path=writefile_path+'_w'+str(window)+'.txt'
    writefile = codecs.open(writefile_path, 'w', 'utf-8')
    # write_wp_file = codecs.open('/save/wenpeng/datasets/LORELEI/wp_filelist.txt', 'w', 'utf-8')
    ...
    # re=0
    #
    '''sorted makes sure that the files in the same order with ls -l'''
    files= sorted(os.listdir(ltf_path))
    # for file in files:
    #     print(file)
    # exit(0)
    print(('folder file sizes: ', len(files)))
    ...
    list_docSegSent=[]
    for fil in files:
        file_id = fil[:fil.find('.json')-2]
        segment_id = 'segment-'+fil[fil.find('.json')-1: fil.find('.json')]
        readfile = codecs.open(ltf_path+fil, 'r', 'utf-8')
        line_co = 0
        for line in readfile:
            if line_co == 3 :
                line_parts = line.strip().split('"')
                assert line_parts[1] == 'text'
                text = line_parts[3]
                break
            line_co+=1
        # print(file_id, text)
        seg_end = file_id_2_start.get(file_id)
        if seg_end is None:
            seg_end = 0
        else:
            seg_end+=2
        print(file_id, segment_id, seg_end,seg_end+len(text.strip()))
        list_docSegSent.append((file_id,str(segment_id), seg_end,seg_end+len(text.strip()), text.strip()))
        seg_end+=len(text.strip())
        file_id_2_start[file_id] = seg_end
    ...
    # list_docSegSent=[]
    # readfile = codecs.open(ltf_path, 'r', 'utf-8')
    # seg_id=0
    # length= 0
    # for line in readfile:
    #     list_docSegSent.append((file_id,str(seg_id), length,length+len(line.strip()), line.strip()))
    #     seg_id+=1
    ...
    if len(list_docSegSent) <=2*window+1:
        ...
        for i, trip in enumerate(list_docSegSent):
            doc_idd = trip[0]
            entity_pos_list = docid2entity_pos_list.get(doc_idd)
            '''it is possible that a doc has no detected entity'''
            if entity_pos_list is None:
                continue
            seg_idd = trip[1]
            seg_start = trip[2]
            seg_end  = trip[3]
...
def get_need_other_fields(matrix):
    ...
    if matrix[0][0]>matrix[0][1] and matrix[0][0]>matrix[0][2]:
        ...
        status = 'future'
        '''it had a bug local variable 'status' referenced before assignment'''
...
def somali_get_type(lines, pred_types, pred_confs, pred_others):
    ...
    # writefile.close()
    # print('official output succeed...Frame size:', frame_size)
    # return 4
...
def load_EDL2018_output(filename):
    ...
    Penn    IL3_SN_000370_20151214_G0T0003YF-0      تۈركىيەدىكى|تۈركىيەدىكى IL3_SN_000370_20151214_G0T0003YF:0-10   NULL    GPE     NAM     1.0

    somali:
    Penn    SOM_DF_001634_20140222_G00202ECB-0      arrimaha dibedda|arrimaha dibedda       SOM_DF_001634_20140222_G00202ECB:67-82  41365861        ORG     NAM     1.0
    Penn    SOM_DF_001634_20140222_G00202ECB-1      Canada|Canada   SOM_DF_001634_20140222_G00202ECB:93-98  5042916 GPE     NAM     1.0
    '''
...
def validate_output_schema(filename, schemafile):
    print(filename)
    try:
    ...
    try:
        ...
        sys.exit('Failed to validate the json output')
