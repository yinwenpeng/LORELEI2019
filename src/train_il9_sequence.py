from load_data import load_BBN_multi_labels_dataset, load_il_groundtruth_as_testset, load_official_testData_il_and_MT, generate_2019_official_output, load_trainingData_types_plus_others,load_trainingData_types_inSequence,load_SF_type_descriptions, average_f1_two_array_by_col, load_fasttext_multiple_word2vec_given_file, load_word2vec_to_init
...
if use_bert:
    bert_tokenizer = BertTokenizer.from_pretrained('/shared/experiments/kkarthi/Extend_mbert/il9/il9_500k')#load the vocab.txt file
    bert_model = BertModel.from_pretrained('/shared/experiments/kkarthi/Extend_mbert/il9/il9_500k')
    bert_model.eval()
    bert_model.to('cuda')
...
def train(task1_data,task2_data,test_data, label_sent, label_mask, id2word, epoch_num, model, loss_function, optimizer):
    ...
    '''
    training_data_index_sequence, training_masks_sequence, training_labels_sequence = task1_data
    training_data_task2_index, training_task2_masks, training_task2_labels, train_task2_other_labels = task2_data
    testing_data_index, testing_masks, testing_labels = test_data
    ...
    iter = 0
    for seq in range(len(training_data_index_sequence)):
        print('training data #', seq, '...')
        training_data_index = training_data_index_sequence[seq]
        training_masks = training_masks_sequence[seq]
        training_labels = training_labels_sequence[seq]
        '''start train on current training data'''
        customized_epoch_num = epoch_num if seq == len(training_data_index_sequence)-1 else 1
        customized_batch_size = config['batch_size'] if seq == len(training_data_index_sequence)-1 else config['batch_size']*1
        for epoch in range(customized_epoch_num):

            print('current epoch: ', epoch, 'at data#', seq)
            minibatches_idx = get_minibatches_idx(len(training_data_index), minibatch_size=customized_batch_size, shuffle=True)
            minibatches_idx_task2 = get_minibatches_idx(len(training_data_task2_index), minibatch_size=customized_batch_size, shuffle=True)
            for i, minibatch in enumerate(minibatches_idx):
                ...
                if iter %20 == 0:
                    ...
                    print('testing....')
                    test(testing_data_index, testing_masks, testing_labels, model, label_sent, label_mask, id2word)
        print('train over.')


def test(training_data_index, training_masks, testing_labels, model, label_sent, label_mask, id2word):
    ...
    with torch.no_grad():
        ...
        for i, minibatch in enumerate(minibatches_idx):
            '''这里是决定一个mibatch之后才去pad, 感觉有点低效'''
            sentence_batch, mask_batch, label_batch= get_mask(training_data_index, training_masks, testing_labels, minibatch)
            sentence_batch = autograd.Variable(torch.cuda.LongTensor(sentence_batch))
            mask_batch = autograd.Variable(torch.cuda.FloatTensor(mask_batch))
...
if __name__ == '__main__':
    ...
    word2id={}
    test_file_path = '/home/wyin3/LORELEI/2019/experiments_daily/il9_uyghur_labeled_as_training_seg_level.txt'
    train_p1_sents, train_p1_masks, train_p1_labels,word2id = load_trainingData_types_inSequence(word2id, config['maxSentLen'])
    ...
    train_sents = [train_p1_sents[0]+train_p1_sents[1], train_p1_sents[1]+train_p2_sents]
    train_masks = [train_p1_masks[0]+train_p1_masks[1], train_p1_masks[1]+train_p2_masks]
    train_labels = [train_p1_labels[0]+train_p1_labels[1], train_p1_labels[1]+train_p2_labels]
    ...
    # print('loading bilingual embeddings....')
    word2vec=load_fasttext_multiple_word2vec_given_file([emb_root+'cleaned-100k-il9longerw2v-multicca.d300.eng.txt',emb_root+'cleaned-100k-il9longerw2v-multicca.d300.il9longerw2v.txt'], 300)
    ...
    test_data=(test_sents, test_masks, test_labels)
