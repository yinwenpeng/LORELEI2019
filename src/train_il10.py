...
def train(task1_data,task2_data,test_data, label_sent, label_mask, id2word, epoch_num, model, loss_function, optimizer):
    ...
    if is_train:
        ...
        torch.save(model.state_dict(), '/scratch/wyin3/dickens_save_dataset/LORELEI/il10/savedmodels/models.il10.pt')
...
def test(training_data_index, training_masks, testing_labels, model, label_sent, label_mask, id2word):
    model.load_state_dict(torch.load('/scratch/wyin3/dickens_save_dataset/LORELEI/il10/savedmodels/models.il10.pt'))
    model.eval()
...
if __name__ == '__main__':
    ...
    config = {"emb_size": 300, "hidden_size": 300, 'epoch_num': 6, 'lr': 0.0003, 'batch_size': 50,
    ...
    use_bert=False
    if use_bert:
        bert_path = '/shared/experiments/kkarthi/nlpgrid_Extend_mbert/Extend_mbert/il10/il10_V1_1M'
        bert_tokenizer = BertTokenizer.from_pretrained(bert_path)#load the vocab.txt file
        bert_model = BertModel.from_pretrained(bert_path)
        bert_model.eval()
        bert_model.to('cuda')
    word2id={}
    test_file_path = '/home/wyin3/LORELEI/2019/experiments_daily/il10_labeled_as_training_seg_level.txt'
